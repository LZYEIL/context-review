...
::: {#correspondence}
✉ — Correspondence possible via GitHub Issues on [github.com/AdaptInfer/context-review](https://github.com/AdaptInfer/context-review/issues)
{% if manubot.authors|map(attribute='corresponding')|select|max -%}
or email to
{% for author in manubot.authors|selectattr("corresponding") -%}
{{ author.name }} \<{{ author.email }}\>{% if loop.last %}.{% elif loop.revindex0 == 1 %} and {% else %}, {% endif %}
{% endfor %}
{% endif %}
:::
